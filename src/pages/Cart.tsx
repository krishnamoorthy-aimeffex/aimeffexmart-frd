...
function Cart() {
 ...
 function incrementQuantity(id: Product["id"]) {
  if (isLoggedIn) {
    ...
    updateQuantity({ id: String(id), quantity: newQty });
 ...
 function removeFromCart(id: Product["id"]) {
  ...
  const total = items.reduce((acc, it) => acc + Number(it.price || 0) * (it.quantity || 1), 0);
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <span className="text-sm text-gray-500">
                        x {product.quantity || 1} = ₹{(Number(product.price || 0) * (product.quantity || 1)).toFixed(2)}
                      </span>
                    </div>

                    {/* Quantity Control */}
                    <div className="flex items-center gap-2 bg-gray-200 rounded-xl p-1 w-fit mb-3">
                      <button
                        onClick={() => (product.quantity || 1) === 1 ? removeFromCart(product.id) : decrementQuantity(product.id)}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-20">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-lg font-bold text-purple-600">{totalQuantity}</span>
              </div>
